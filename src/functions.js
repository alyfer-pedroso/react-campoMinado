...
const spreadMines = (board, minesAmount) => {
  ...
  while (minesPlanted < minesAmount) {
    const randomRowSelected = parseInt(Math.random() * rows, 10);
    const randomColumnSelected = parseInt(Math.random() * columns, 10);

    if (!board[randomRowSelected][randomColumnSelected].mined) {
      board[randomRowSelected][randomColumnSelected].mined = true;
      minesPlanted++;
    }
  }
};
